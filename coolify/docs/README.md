...
## Overview
This deployment package provides a complete Node.js + MySQL stack for the Rigways Asset & Certificate Management System, optimized for Coolify deployment.
...
3. Upload or paste the `docker/docker-compose.yml` content
...
- Environment variables:
  ...
  - `MYSQL_DATABASE`: rigways_acm
  - `MYSQL_USER`: rigways_user
  - `MYSQL_PASSWORD`: your_secure_password
- Mount volume: `/var/lib/mysql`
...
## Database Schema
SQL files are automatically initialized on first run from:
- `sql/001_schema.sql` - Main tables (MySQL 8.0 compatible)
- `sql/002_certificate_history.sql` - Certificate history triggers
...
## Frontend
All frontend files are located in `frontend/` directory and served automatically by the backend.
...
- `inspectors.html` - Inspectors management
...
```bash
docker-compose exec mysql mysql -u rigways_user -p rigways_acm
```

## Migration Notes

Key differences from the original Cloudflare Worker + Supabase:
1. **Database**: PostgreSQL → MySQL 8.0 (schema adapted)
2. **UUID**: `gen_random_uuid()` → `UUID()` function
3. **TIMESTAMPTZ**: → `TIMESTAMP`
4. **JSONB**: → `JSON`
5. **Auto-increment numbers**: Handled in application code instead of triggers
6. **Storage**: Cloudflare R2 → Local filesystem (`uploads/` directory)
7. **Runtime**: Cloudflare Workers → Node.js Express

All business logic, permissions, and API contracts remain identical.

## Support
For issues or questions, check the application logs or review the original implementation.
