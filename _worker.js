...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path.startsWith('/ai')) return await handleAI(request, env, path);
...
// ── ai.js ──
// worker/src/routes/ai.js — OpenRouter AI integration with Qwen model
...
async function handleAI(request, env, path) {
  ...
  const session = await getSession(request, env);
  if (!session) return unauth(env, request);

  /* ── POST /api/ai/chat — Send message to AI assistant ── */
  if (path === '/ai/chat' && method === 'POST') {
    try {
      const body = await request.json();
      const { messages, model = 'qwen/qwen-3.5-instruct' } = body;

      if (!messages || !Array.isArray(messages)) {
        return badReq('Messages array is required', 'INVALID_REQUEST', env, request);
      }
      ...
      if (!apiKey) {
        return json({ 
          success: false, 
          error: 'AI not configured. Please set OPENROUTER_API_KEY in Worker environment variables.',
      ...
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'Content-Type': 'application/json',
          'HTTP-Referer': 'https://rigways.pages.dev',
          'X-Title': 'Rigways ERP Assistant'
        },
      ...
      if (!response.ok) {
        ...
        if (response.status === 402) {
          return json({ 
            success: false, 
            error: 'Insufficient credits in OpenRouter account. Please top up your account.',
      ...
      console.log(`AI Usage - Tokens: ${usage.total_tokens || 'N/A'}, Model: ${model}`);
      ...
      return json({ 
        success: false, 
        error: 'AI processing error: ' + (error.message || String(error)),
  ...
  if (path === '/ai/status' && method === 'GET') {
    ...
    return ok({
      ...
      model: 'qwen/qwen-3.5-instruct',
      provider: 'OpenRouter',
      message: isConfigured 
        ? 'AI is properly configured and ready to use.'
        : 'AI not configured. Please add OPENROUTER_API_KEY to Worker environment variables.'
    }, env, request);
  }

  return badReq('AI route not found', 'NOT_FOUND', env, request);
}
